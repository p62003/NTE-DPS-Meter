...
置顶透明卡片，圆形角色头像 + 呼吸光环 + 三行实时数据。战斗时自动展开，5 秒无命中自动收回。战斗中鼠标自动穿透，不影响游戏操作。
...
自动识别当前操作角色，支持全 17 角色。4 人队伍中每个角色独立追踪伤害。
...
- **Connect 绑定** — 贴上 Device ID 或 Analytics Token 即可在网页查看个人数据
- **三语网站** — EN / 繁體中文 / 简体中文
- **隐私友好** — 上传前须明确同意，数据匿名化

### 主窗口（赞助者）

一目了然的全队伤害贡献 — 角色排名栏 + 命中细节栏双栏布局。所有数据保存在本地。

![Main Window](images/main_window_EN.png)

### 战报对比（赞助者）

A/B 并排对比不同队伍配置，显示伤害与 DPS 差异摘要。
