**[English](README.md)** | **繁體中文** | **[简体中文](README_CN.md)** | **[日本語](README_JA.md)**
...
自動識別當前操作角色，支援全部已上線角色。4 人隊伍中每個角色獨立追蹤傷害。同時自動辨識遊戲模式（危險委託 / 深淵環線 / 異象巡禮 / 追獵 / 999 夜），記錄於戰報並配專屬圖標。

### 戰報與戰報管理

按 **F6** 重置統計 — 當前戰鬥自動存為戰報。從右鍵選單開啟戰報管理，可依傷害範圍與角色 / 目標分組篩選、排序、上傳至社群平台或刪除。戰報存檔後會自動於背景上傳（免費用戶恆開，贊助者可於右鍵選單開關）。

![Report Manager](images/report_manager_EN.png)

### 主視窗與傷害心電圖（ECG）

一目了然的全隊傷害貢獻 — 角色排名欄 + 戰鬥細節（「施放 / 擊中」雙分頁：技能分組與逐次命中） + 傷害心電圖三區佈局。所有資料保存在本地。
...
22 角色養成素材一覽 — 突破材料、技能升級（含被動）、好感度送禮三策略。
...
- **簡易工具 — 隊伍搭配器** — 免費、不需登入：點角色卡填入四格，即時列出 8 種異能環合反應的覆蓋狀況
- **Discord 登入** — 用 Discord 帳號登入網頁端，贊助者可直接驗證序號
- **贊助購買** — 海外用戶可於 [ntedpsmeter.com/purchase](https://ntedpsmeter.com/purchase) 以 PayPal / Ko-fi 自助贊助，付款後自動 email 序號（6 / 11 / 15 USD 對應 30 / 60 / 90 天）；台灣用戶請洽 Discord
- **五語網站** — EN / 繁體中文 / 简体中文 / 日本語 / 한국어
- **隱私友善** — 上傳前須明確同意，數據匿名化
...
- **全用戶 Discord 登入** — 免費與贊助用戶啟動時皆完成一次 Discord 登入（已綁定者不受影響），序號綁定擁有者防止盜用
- **戰報自動上傳** — 戰報存檔後自動於背景上傳，免費恆開、贊助可開關
- **紅點提示** — 新戰報入庫或新版本可更新時，以紅點指引到對應功能
- **首次啟動語系偵測** — 依 Windows 系統語系自動選擇介面語言，手動改過者維持選擇
- **快捷鍵** — F6 重置、Alt+D 浮窗、Alt+E 主視窗；可完全自訂
- **網卡選擇** — 手動切換網路介面，適用加速器/VPN 環境；持續支援 ExitLag
- **四語系介面** — 繁體中文 / 简体中文 / English / 日本語（下拉選單直選或右鍵選單輪替）
- **系統匣常駐** — 不佔工作列
- **自動更新** — 從右鍵選單檢查更新
- **推薦卡片** — 主視窗、角色養成、抽卡紀錄、戰報管理、個人數據中心五個畫面右側的可關閉卡片，內容是作者自己的另一款遊戲；不會出現在戰鬥浮窗上
...
贊助者序號是支持開發的感謝回饋，不是訂閱服務。每組序號依方案有效期 **30 / 60 / 90 天**。免費功能永遠不受限制。
